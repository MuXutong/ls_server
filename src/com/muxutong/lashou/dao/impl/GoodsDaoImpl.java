...
public class GoodsDaoImpl extends BaseDao implements GoodsDao {
	...
	public int getCountByLBS(String category, double lat, double lon, double minLat, double minLon, double maxLat,
		...
		String sql = "select count(*) as count"
				+ " from prodouct p,shop s where p.shop_id=s.shop_id";
		if(category==null||"".equals(category)){//为空是即代表查询全部
			...
			sql+= " and s.shop_lat BETWEEN '" + minLat + "' and '"
				+ maxLat + "' and s.shop_lon BETWEEN '" + minLon
				+ "' and '" + maxLat + "'";
		// 定义数据库链接
		Connection connection = null;
		// 定义sql语句执行的对象
	...
	public List<Goods> getGoodByLBS(int page, int size, String category, double lat, double lon, double minLat,
		...
		String sql = "select "
				+ "GLength(LineString(PointFromWKB(POINT("+lat+","+lon+")),PointFromWKB(POINT(s.shop_lat,s.shop_lon))))*69*1609.344 AS distance,"
